...
var
  ...
  LSalt: string;
begin
  LBcrypt := TBcrypt.Create;
  try
    LRound := StrToIntDef(Copy(ASalt, 5, 2), 10);
    LSalt := Copy(ASalt, 8, 22);
    LPassword := LBcrypt.HashPassword(APassword, LSalt, LRound);
    Result := LBcrypt.Base64Encode(LPassword);
  finally
    LBcrypt.Free;
  end;
end;
...
function TBCrypt.Base64Decode(const Str: string): string;
var
  LBase64: TBase64Encoding;
begin
  LBase64 := TBase64Encoding.Create(0);
  try
    Result := LBase64.Decode(Str);
  finally
    LBase64.Free;
  end;
end;

function TBCrypt.Base64Encode(const Str: string): string;
var
  LBase64: TBase64Encoding;
begin
  LBase64 := TBase64Encoding.Create(0);
  try
    Result := LBase64.Encode(Str);
  finally
    LBase64.Free;
  end;
end;

function TBcrypt.BsdBase64Decode(const s: string): TBytes;
...
function TBcrypt.BsdBase64Encode(const Data: TBytes; len: integer): string;
var
  i: integer;
  b1, b2: integer;
begin
...
function TBcrypt.CheckPassword(const Str: string; const Hash: string; const rounds: TRoundsCount = 10): Boolean;
var
  RegexObj: TRegEx;
  match   : TMatch;
begin
...
function TBcrypt.CryptRaw(const key, salt: TBytes; const rounds: integer): TBytes;
var
...
procedure TBcrypt.EKSKey(const salt, key: TBytes);
var
...
{$OVERFLOWCHECKS OFF}
procedure TBcrypt.Encipher(var lr: array of DWord; const offset: integer);
var
  i, n, block, r: DWord;
begin
  block := lr[offset];
...
function TBcrypt.FormatPasswordHashForBsd(const salt, hash: TBytes; const rounds: TRoundsCount): string;
var
  LSalt: string;
  LHash: string;
begin
  LSalt := BsdBase64Encode(salt, Length(salt));
  LHash := BsdBase64Encode(hash, Length(MagicText) * 4 - 1);
  Result := Format('$2a$'+FormatFloat('00', rounds)+'$%s%s', [LSalt, LHash]);
end;

function TBcrypt.GenerateSalt: string;
const
...
function TBcrypt.HashPassword(const Str: string; const salt: string; const rounds: TRoundsCount = 10): string;
var
  LPassword: string;
  key, saltBytes, Hash: TBytes;
  i: integer;
begin
  LPassword := String(AnsiToUtf8(str));
  SetLength(key, Length(LPassword) + 1);
  for i := 0 to length(LPassword) - 1 do
    key[i] := ord(LPassword[i+1]);
...
function TBcrypt.HashPassword(const Str: string; const rounds: TRoundsCount = 10): string;
begin
  Result := HashPassword(Str, GenerateSalt, rounds);
end;

procedure TBcrypt.InitializeKey();
begin
  Move(SBoxOrg, SBox, Sizeof(SBox));
  Move(PBoxOrg, PBox, Sizeof(PBox));
end;

procedure TBcrypt.NKey(const key: TBytes);
var
  lr: array[0..1] of DWord;
...
function TBcrypt.StreamToWord(const Data: TBytes; var offset: integer): DWord;
var
  i: integer;
begin
  Result := 0;
